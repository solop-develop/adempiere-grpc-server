...
public class PExchangeDifference extends SvrProcess {
    ...
    //Solop. Nicolas Sarlabos. 11/03/2021. #15747.
    ...
    protected void prepare() {
        ...
        for (int i = 0; i < para.length; i++)
        {
            ...
            if (para[i].getParameter() == null)
                ...
                p_SalesRegion_ID = para[i].getParameterAsInt();
            else
    ...
    protected String doIt() throws Exception {
        ...
        if(batch != null)
            rePost();//Solop. Nicolas Sarlabos. 23/02/2021. #15616.

        return "#" + noT + info;
    }

    /***
     * Metodo que vuelve a contabilizar los journal.
     * Esto se requiere debido al cambio en los montos fuente de las lineas luego de completarse el journal.
     * Solop. Issue #15616
     * @author Nicolas Sarlabos - 23/02/2021
     */
    private void rePost() {
    ...
    private String createGLJournal()
    {
        ...
        batch.setC_Currency_ID(p_C_Currency_ID);//Openup. Nicolas Sarlabos. 10/09/2020. #14650.
        ...
        try{
            ...
            while(rs.next()){
                ...
                //Solop. Nicolas Sarlabos. 11/03/2021. #15747.
                if(p_User1_ID > 0) line.setUser1_ID(p_User1_ID);
                if(p_SalesRegion_ID > 0) line.setC_SalesRegion_ID(p_SalesRegion_ID);
                //Fin #15747.
            ...
            //Openup. Nicolas Sarlabos. 10/09/2020. #14650.
            DB.executeUpdateEx("update gl_journalline set amtsourcecr = 0, amtsourcedr = 0 where gl_journal_id = " + journal.get_ID(), get_TrxName());

            //Solop. Nicolas Sarlabos. 11/03/2021. #15747.
            if(p_User1_ID > 0) DB.executeUpdateEx("update gl_journalline set user1_id = " + p_User1_ID + " where gl_journal_id = " + journal.get_ID(), get_TrxName());
            if(p_SalesRegion_ID > 0) DB.executeUpdateEx("update gl_journalline set c_salesregion_id = " + p_SalesRegion_ID + " where gl_journal_id = " + journal.get_ID(), get_TrxName());
            //Fin #15747.
                                  ...
                                  BigDecimal dr, BigDecimal cr, int orgId, int lineNo)
    {
        ...
        if (dr.compareTo(cr) > 0)
        {
            ...
            MAccount acct = MAccount.get(getCtx(), acctSchemaDefault.getAD_Client_ID(), orgId,
                    ...
                    base.getUserElement1_ID(), base.getUserElement2_ID(), 0, get_TrxName());
