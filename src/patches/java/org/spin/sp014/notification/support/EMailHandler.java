 ...
 */
public final class EMailHandler implements Serializable
{
	 ...
	 * @param message
	 */
	public EMailHandler (MClient client, String from, String to, 
	 ...
	 */
	public EMailHandler (MClient client, int eMailConfigId, String from, String to, 
	 ...
	 * @param eMailConfigId
	 */
	public EMailHandler (MClient client, int eMailConfigId) {
		//	FR [ 402 ]
	 ...
	 * @param client
	 */
	public EMailHandler (MClient client) {
	...
	/**	Logger							*/
	protected static CLogger		log = CLogger.getCLogger (EMailHandler.class);
	...
	{
		//	Local Character Set
		String charSetName = "utf-8";//Ini.getCharset().name();
		...
		if (attachments == null || attachments.size() == 0)
		{
			if (messageHTML == null || messageHTML.length () == 0)
				...
				msg.setDataHandler (new DataHandler
					(new ByteArrayDataSource (messageHTML, charSetName, "text/html; charset=" + charSetName)));
			//
	...
	public static String validateMailDelivery(Properties ctx, String requestEMail, String requestPassword, int eMailConfigId, String name) {
		...
		//
		EMailHandler email = new EMailHandler(MClient.get(ctx), eMailConfigId, requestEMail, requestEMail,
		...
		try
		{
			...
			if (EMailHandler.SENT_OK.equals (returnMessage))
			{
