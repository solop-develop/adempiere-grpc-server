...
import org.spin.sp014.util.ERPCloundNotification;
	...
	public void sendNotification(MADNotificationQueue notification) {
		...
		notification.getRecipients().forEach(recipient -> {
			...
			if (eMailConfigurationId <= 0 || eMailUser == null || eMailUser.isEmpty() || eMailPassword == null || eMailPassword.isEmpty()) {
				...
				//	Create instance
				EMailHandler email = new EMailHandler(client, eMailConfigurationId);
				email.setFrom(eMailUser);
				email.addTo(recipient.getAccountName());
				if (!email.isValid() && !email.isValid(true)) {
					...
					if(!Util.isEmpty(notificationSignature)) {
						notificationSignature = NotificationParser.newInstance(notification.getCtx())
								.withPO(notification)
								.withPO(MClient.get(notification.getCtx(), notification.getAD_Client_ID()))
								.withPO(MOrg.get(notification.getCtx(), notification.getAD_Org_ID()))
								.withUser(recipient.getAD_User_ID())
								.parse(notificationSignature);
						textWithSignature
							.append("<p>")
							.append(notificationSignature)
							.append("</p>")
							;
					}
					email.setMessageHTML(textWithSignature.toString());
					//	Attachment
