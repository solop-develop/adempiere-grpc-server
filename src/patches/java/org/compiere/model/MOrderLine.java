...
public class MOrderLine extends X_C_OrderLine implements IDocumentLine
{
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		//	Apply FlatDiscount from header on new lines
		BigDecimal parentFlatDiscount = getParent().getFlatDiscount();
		if (newRecord && getDiscount().signum() == 0) {
			if (parentFlatDiscount.signum() != 0) {
				setDiscount(parentFlatDiscount);
			}
		}
		//Project
				...
				if(!getParent().isReturnOrder()) {
					...
					if(newRecord) {
						if ((getPriceActual() == null || getPriceActual().signum() == 0) && getDiscount().compareTo(Env.ONEHUNDRED) != 0) {
							if (getDiscount().signum() != 0 && getPriceList().signum() != 0 && parentFlatDiscount.signum() != 0) {
