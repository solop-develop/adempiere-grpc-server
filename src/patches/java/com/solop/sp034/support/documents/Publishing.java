...
public class Publishing implements IMercadoLibreDocument {
    ...
    public Publishing(PO publishing) {
        ...
        availableQuantity = (BigDecimal) publishing.get_Value(Changes.SP034_CurrentInventory);
    ...
    public Map<String, Object> getData() {
        ...
        // publishing flow can update the existing item instead of creating a duplicate.
        String publicationCode = publishing.get_ValueAsString("SP034_PublicationCode");
        if(!Util.isEmpty(publicationCode, true)) {
            publishValues.put("publication_code", publicationCode);
        }
        publishValues.put("attributes", getAttributesWithSellerSku());
        return publishValues;
    }

    /**
     * Attributes sent to MercadoLibre, ensuring SELLER_SKU = product value.
     * SELLER_SKU lets MercadoLibre (and the publishing flow) identify the item by
     * the seller's own code, which enables de-duplication on re-publish.
     */
    private List<Map<String, Object>> getAttributesWithSellerSku() {
        List<Map<String, Object>> attributes = getAttributes();
        String sellerSku = product.getValue();
        if(!Util.isEmpty(sellerSku, true)
                && attributes.stream().noneMatch(attribute -> "SELLER_SKU".equals(attribute.get("id")))) {
            Map<String, Object> sellerSkuAttribute = new HashMap<>();
            sellerSkuAttribute.put("id", "SELLER_SKU");
            sellerSkuAttribute.put("value", sellerSku);
    ...
    private List<Map<String, Object>> getAttributes() {
        ...
        new Query(
            ...
            .forEach(allocationId -> {
                ...
                String name = null;
                if(type == null) {
                    String strValue = attributeAllocation.get_ValueAsString("SP034_String");
                    value = Util.isEmpty(strValue, true) ? null : strValue;
                    ...
                    String strValue = attributeAllocation.get_ValueAsString("SP034_LongString");
                    value = Util.isEmpty(strValue, true) ? null : strValue;
