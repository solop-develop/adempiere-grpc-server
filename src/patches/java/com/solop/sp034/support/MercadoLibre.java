...
public class MercadoLibre implements IWebhook {
    ...
    private Map<String, Object> getAttributes(PO entity) {
        ...
        // The per-country MercadoLibre flow identifies the publishing store to pick the right
        // credentials/account from the client list. W_Store_ID is not unique across tenants, so the
        // store UUID (matched against the sheet's store_uuid column) and its name are sent. The
        // client name is also sent so the store can be recognized at a glance in n8n.
        int storeId = entity.get_ValueAsInt("W_Store_ID");
        if(storeId > 0) {
            MStore store = MStore.get(entity.getCtx(), storeId);
            if(store != null) {
                data.put("store_id", store.getW_Store_ID());
                data.put("store_uuid", store.getUUID());
                data.put("store_name", store.getName());
            }
        }
        data.put("client_name", MClient.get(entity.getCtx(), entity.getAD_Client_ID()).getName());
        return data;
    }
    ...
    private void addMetadata(JsonObject requestBodyJson, PO entity) {
        ...
        int storeId = entity.get_ValueAsInt("W_Store_ID");
        requestBodyJson.addProperty("store_id", storeId);
        // store_uuid at the body root: the per-country flow looks up the store credentials in the
        // client sheet by this value ($json.body.store_uuid), so it must travel at the top level,
        // not only inside the attributes object.
        if(storeId > 0) {
            MStore store = MStore.get(entity.getCtx(), storeId);
            if(store != null) {
        ...
        requestBodyJson.addProperty("client_id", entity.get_ValueAsInt("AD_Client_ID"));
