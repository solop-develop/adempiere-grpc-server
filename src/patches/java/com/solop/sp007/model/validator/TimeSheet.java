...
public class TimeSheet implements ModelValidator {
	...
	public String modelChange(PO entity, int type) throws Exception {
		...
		if(entity.get_TableName().equals(I_S_ResourceAssignment.Table_Name)) {
			...
			//	Recalculate R_Request.TimeSpent from the sum of active assignments Qty
			if (type == TYPE_AFTER_NEW || type == TYPE_AFTER_CHANGE || type == TYPE_AFTER_DELETE) {
				int requestId = entity.get_ValueAsInt("R_Request_ID");
				TimeSheetControl.updateRequestTimeSpent(entity.getCtx(), requestId, entity.get_TrxName());
				//	If the assignment was reassigned to another request, recalculate the previous one too
				if (type == TYPE_AFTER_CHANGE && entity.is_ValueChanged("R_Request_ID")) {
					int oldRequestId = entity.get_ValueOldAsInt("R_Request_ID");
					if (oldRequestId != requestId) {
						TimeSheetControl.updateRequestTimeSpent(entity.getCtx(), oldRequestId, entity.get_TrxName());
					}
				}
			}
		}
