...
public class Invoicy_v1 implements IFiscalSender, IGetElectronicInvoices {
    ...
    protected String pushInvoiceUrl;
    protected String getInvoiceUrl;
    protected String downloadInvoiceUrl;
    protected String clientCode;
    protected String clientPK;
    protected String clientCK;
    ...
    protected int getDocumentId(IFiscalDocument fiscalDocument) {
    ...
    protected String getDocumentColumnName(IFiscalDocument fiscalDocument) {
    ...
    protected String getLastFiscalDocumentNo(int documentId, String columnName) {
            ...
            for (CFEInvoiCyType.Detalle.Item line : item.getDetalle().getItem()) {
                try {
                    Trx.run(trxName -> {
                        ...
                        String queryBP = I_C_BPartner.COLUMNNAME_TaxID + "=? AND AD_Org_ID IN (0,?)";
                        MBPartner businessPartner = new Query(Env.getCtx(), I_C_BPartner.Table_Name, queryBP, trxName)
                            .setParameters(emisor.getEmiRut(), orgId)
                        ...
                        if (businessPartner != null) {
                            ...
                            int paymentTermId = businessPartner.getPO_PaymentTerm_ID();
                            if(paymentTermId > 0){
                                MPaymentTerm paymentTerm = new MPaymentTerm(Env.getCtx(), paymentTermId, trxName);
                                iInvoice.setPaymentTermValue(paymentTerm.getValue());
                                iInvoice.setC_PaymentTerm_ID(paymentTerm.get_ID());
                            } else {
                        ...
                        // Only assign productValue if a charge was not obtained from the business partner
                        if (iInvoice.getC_Charge_ID() <= 0) {
                        ...
                        BigDecimal priceActualAmt = BigDecimal.ZERO;
                        if (line.getIteCantidad() != null && line.getIteCantidad().compareTo(BigDecimal.ZERO) != 0) {
                            priceActualAmt = line.getIteMontoItem().divide(line.getIteCantidad(), priceList.getStandardPrecision(), RoundingMode.HALF_UP);
                        }
    ...
    protected String getPushInvoiceUrl() {
        return pushInvoiceUrl;
    }

    protected String getGetInvoiceUrl() {
        return getInvoiceUrl;
    }

    protected String getDownloadInvoiceUrl() {
    ...
    protected String getClientPK() {
        return clientPK;
    }

    protected String getClientCK() {
        return clientCK;
    }
}
