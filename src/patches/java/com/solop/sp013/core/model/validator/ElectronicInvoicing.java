...
public class ElectronicInvoicing implements ModelValidator {
	...
	public String modelChange(PO po, int type) throws Exception {
		...
		boolean isElectronicInvoicingOrg = orgInfo.get_ValueAsBoolean(ElectronicInvoicingChanges.SP013_IsElectronicInvoicing);

		//	Billing Criteria propagation and Electronic Line Summary grouping only require the
		//	target DocType to be electronic, they do not depend on the Org-level Electronic Invoicing flag
		if (po.get_TableName().equals(I_C_Invoice.Table_Name)) {
			MInvoice invoice = (MInvoice) po;
			if ((type == TYPE_BEFORE_NEW ||
					(type == TYPE_BEFORE_CHANGE && invoice.is_ValueChanged(I_C_Invoice.COLUMNNAME_C_BPartner_ID)))
					&& invoice.isSOTrx()) {
				MDocType documentTypeForCriteria = MDocType.get(invoice.getCtx(), invoice.getC_DocTypeTarget_ID());
				if (documentTypeForCriteria != null && documentTypeForCriteria.get_ValueAsBoolean(ElectronicInvoicingChanges.SP013_IsElectronicDocument)) {
					MBPartner mbPartner = MBPartner.get(invoice.getCtx(), invoice.getC_BPartner_ID());
					...
					if (!Util.isEmpty(electronicBillingCriteria, true)) {
						invoice.set_ValueOfColumn("SP013_BillingCriteria", electronicBillingCriteria);
					}
				}
			}
			if (type == TYPE_AFTER_CHANGE) {
				...
				if (documentType != null && documentType.get_ValueAsBoolean(ElectronicInvoicingChanges.SP013_IsElectronicDocument)
						&& invoice.is_ValueChanged("SP013_BillingCriteria")) {
		...
		if(type == TYPE_BEFORE_NEW
			...
			if (po.get_TableName().equals(MInvoice.Table_Name)) {
				...
				if ((type == TYPE_BEFORE_NEW || invoice.is_ValueChanged(MInvoice.COLUMNNAME_C_Order_ID))
					&& invoice.getC_Order_ID() > 0){
					MOrder order = (MOrder) invoice.getC_Order();
					String orderFiscalComment = order.get_ValueAsString(ElectronicInvoicingChanges.SP013_FiscalComment);
					if (!Util.isEmpty(orderFiscalComment, true)){
						String invoiceFiscalComment = invoice.get_ValueAsString(ElectronicInvoicingChanges.SP013_FiscalComment);
						if (Util.isEmpty(invoiceFiscalComment, true)){
							invoice.set_ValueOfColumn(ElectronicInvoicingChanges.SP013_FiscalComment, orderFiscalComment);
						} else {
							invoice.set_ValueOfColumn(ElectronicInvoicingChanges.SP013_FiscalComment, invoiceFiscalComment + " | " + orderFiscalComment);
						}
					}
				}
				if(invoice.isSOTrx()) {
					...
					if (type == TYPE_BEFORE_NEW) {
						...
						String invoiceDocumentNote = orgInfo.get_ValueAsString(ElectronicInvoicingChanges.SP013_InvoiceDocumentNote);
						if (!Util.isEmpty(invoiceDocumentNote, true)) {
							String invoiceFiscalComment = invoice.get_ValueAsString(ElectronicInvoicingChanges.SP013_FiscalComment);
							if (Util.isEmpty(invoiceFiscalComment, true)) {
								invoice.set_ValueOfColumn(ElectronicInvoicingChanges.SP013_FiscalComment, invoiceDocumentNote);
							} else {
								invoice.set_ValueOfColumn(ElectronicInvoicingChanges.SP013_FiscalComment, invoiceFiscalComment + "\n" + invoiceDocumentNote);
							}
						}
...
//					}
	...
	public String docValidate(PO po, int timing) {
		...
		//	Electronic Line Summary grouping only requires the target DocType to be electronic,
		//	it does not depend on the Org-level Electronic Invoicing flag
		if (timing == TIMING_BEFORE_PREPARE && po.get_TableName().equals(I_C_Invoice.Table_Name)) {
			MInvoice invoice = (MInvoice) po;
			MDocType documentType = MDocType.get(invoice.getCtx(), invoice.getC_DocTypeTarget_ID());
			if (documentType != null && documentType.get_ValueAsBoolean(ElectronicInvoicingChanges.SP013_IsElectronicDocument)
		...
		if(timing == TIMING_BEFORE_REVERSECORRECT
			...
			log.fine(" TIMING_AFTER_VOID");
			if(po.get_TableName().equals(I_C_Invoice.Table_Name)) {
				MInvoice invoice = (MInvoice) po;
				MDocType documentType = MDocType.get(invoice.getCtx(), invoice.getC_DocType_ID());
				//	When a sent electronic document is voided, ask the provider to cancel it at SUNAT
				//	. Providers that cancel only via credit notes do not implement
				//	IFiscalDocumentCanceller, so this is a no-op for them.
				if(documentType.get_ValueAsBoolean(ElectronicInvoicingChanges.SP013_IsElectronicDocument)
						&& invoice.get_ValueAsBoolean(ElectronicInvoicingChanges.SP013_IsSent)) {
					int fiscalSenderId = MOrgInfo.get(invoice.getCtx(), invoice.getAD_Org_ID(), null).get_ValueAsInt(ElectronicInvoicingChanges.SP013_FiscalSender_ID);
					if(fiscalSenderId > 0) {
						try {
							MADAppRegistration registeredApplication = MADAppRegistration.getById(invoice.getCtx(), fiscalSenderId, null);
							...
							if(supportedApplication != null
									&& IFiscalDocumentCanceller.class.isAssignableFrom(supportedApplication.getClass())) {
								IFiscalDocument fiscalDocument = FiscalDocumentBuilder.fromInvoice(invoice, fiscalSenderId);
								String reason = invoice.get_ValueAsString(ElectronicInvoicingChanges.SP013_FiscalComment);
								if(Util.isEmpty(reason, true)) {
									reason = invoice.getDescription();
								}
								IFiscalSenderResponse result = ((IFiscalDocumentCanceller) supportedApplication).cancelFiscalDocument(fiscalDocument, reason);
								if(result.isError()) {
									return Msg.parseTranslation(Env.getCtx(), "@Error@") + " " + result.getErrorMessage();
								}
							}
						} catch (Exception e) {
