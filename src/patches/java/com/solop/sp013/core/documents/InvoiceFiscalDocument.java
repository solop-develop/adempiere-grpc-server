...
public class InvoiceFiscalDocument implements IFiscalDocument {
    ...
    private void convertDocument(MInvoice document) {
        ...
        if(documentType.get_ValueAsString(ElectronicInvoicingChanges.SP013_FiscalDocumentType)
            ...
            if(invoiceToAllocateId > 0) {
                ...
                addReversalDocument(allocatedDocument, document.getGrandTotal());
            } else {
                String whereClause = "C_Invoice_ID = ?";
                List<PO> allocateInvoices = new Query(document.getCtx(), "C_AllocateInvoice", whereClause, document.get_TrxName())
                    .setParameters(document.getC_Invoice_ID())
                    .list();
                HashMap<Integer, BigDecimal> referencedInvoices = new HashMap<>();
                allocateInvoices.forEach( allocateInvoice -> {
                    int invoiceId = allocateInvoice.get_ValueAsInt("ReferenceDocument_ID");
                    BigDecimal processedAmount = referencedInvoices.get(invoiceId);
                    if (processedAmount == null) {
                        processedAmount = BigDecimal.ZERO;
                    }
                    BigDecimal allocateAmount = (BigDecimal) allocateInvoice.get_Value("AllocateAmount");
                    processedAmount = processedAmount.add(Optional.ofNullable(allocateAmount).orElse(BigDecimal.ZERO));
                    referencedInvoices.put(invoiceId, processedAmount);
                });
                referencedInvoices.forEach((key, value) -> {
                    int allocatedInvoiceId = key;
                    MInvoice allocatedDocument = new MInvoice(document.getCtx(), allocatedInvoiceId, document.get_TrxName());
                    addReversalDocument(allocatedDocument, value);
                });
            }
            ...
            addReversalDocument(reversalDocument, document.getGrandTotal().abs());
        ...
        if(document.getC_PaymentTerm_ID() != 0) {
            ...
            if (paymentTerm.isDueFixed()) {
                ...
                dueCalendar.setTime(document.getDateInvoiced());
                int monthOffset = paymentTerm.getFixMonthOffset();
                if (monthOffset < 0) {
                    monthOffset = 0;
                }
                // Past the cutoff day the due date rolls over to the following month
                if (dueCalendar.get(Calendar.DAY_OF_MONTH) > paymentTerm.getFixMonthCutoff()) {
                    monthOffset++;
                }
                dueCalendar.add(Calendar.MONTH, monthOffset);
                dueCalendar.set(Calendar.DAY_OF_MONTH, paymentTerm.getFixMonthDay());
                // If the fixed day already passed within the invoice month, move to the next month
                if (dueCalendar.getTimeInMillis() < document.getDateInvoiced().getTime()) {
                    dueCalendar.add(Calendar.MONTH, 1);
                }
                this.dueDate = new Timestamp(dueCalendar.getTimeInMillis());
                this.daysDue = TimeUtil.getDaysBetween(document.getDateInvoiced(), this.dueDate);
            } else {
     ...
     */
    public final InvoiceFiscalDocument addReversalDocument(MInvoice reversalDocument, BigDecimal amountToAllocate) {
        ReversalDocument newReversalDocument = new ReversalDocument(reversalDocument);
        newReversalDocument.withReversalAmount(amountToAllocate);
        fiscalReversalDocuments.add(newReversalDocument);
        return this;
    }
    ...
    public PaymentRule getPaymentRule() {
        return isCredit ? PaymentRule.CREDIT : PaymentRule.IMMEDIATE;
    }
    ...
    @Override public String getWarehouseAddress() { return null; }
    @Override public int getNumberOfPackages() { return 0; }
    @Override public String getCarrierName() { return null; }
    @Override public String getCarrierTaxId() { return null; }
