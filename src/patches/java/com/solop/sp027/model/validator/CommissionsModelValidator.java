...
import com.solop.sp027.util.AgencyChanges;
...
public class CommissionsModelValidator implements ModelValidator {
		...
		if(timing == TIMING_AFTER_COMPLETE) {
			if(entity.get_TableName().equals(I_C_Order.Table_Name)) {
				...
				String whereClause = "EXISTS (SELECT 1 FROM SP027_CommissionDocType cdt WHERE cdt.C_Commission_ID = C_Commission.C_Commission_ID " +
						" AND cdt.IsActive='Y' AND cdt.C_DocType_ID = ?)";
			...
			if(entity.get_TableName().equals(I_C_Invoice.Table_Name)) {
				...
				List<Integer> commissionIds = new Query(invoice.getCtx(), MCommission.Table_Name, whereClause, invoice.get_TrxName())
						...
						.setOnlyActiveRecords(true)
						.getIDsAsList();
				AtomicInteger reverseDocTypeId = new AtomicInteger(0);
				...
				commissionIds.forEach(commissionId -> {
					...
					if (commissionDefinition.getC_DocTypeReverse_ID() <= 0) {
						throw new AdempiereException("@C_DocTypeReverse_ID@ @NotFound@");
					}
					...
					if(commissionRun.processIt(MCommissionRun.DOCACTION_Complete)) {
						...
						reverseDocTypeId.set(commissionDefinition.getC_DocTypeReverse_ID());
					}
				});
				if (totalCommissionAmount.get() != null && totalCommissionAmount.get().signum() != 0) {
					reversePreviousCommissionOrders(invoice, totalCommissionAmount.get(), reverseDocTypeId.get());
				}
	...
	private void createOrderFromCommission(MCommission commissionDefinition, MCommissionRun commissionRun, MOrder originalOrder, MInvoice originalInvoice) {
		...
		int currencyId = 0;
		boolean useDocumentCurrency = commissionDefinition.isUseDocumentCurrency();
		if (originalOrder != null && originalInvoice == null) {
			...
			if (useDocumentCurrency) {
				currencyId = originalOrder.getC_Currency_ID();
			}
		}
		if (originalInvoice != null) {
			...
			if (useDocumentCurrency) {
				currencyId = originalInvoice.getC_Currency_ID();
			}
		}
		...
		if (currencyId <= 0) {
			if (useDocumentCurrency) {
				throw new AdempiereException("@IsUseDocumentCurrency@: @C_Currency_ID@ @NotFound@");
			}
		...
		//	Get from DB
		final String whereClause = "IsSOPriceList=? AND C_Currency_ID=? AND IsTaxIncluded=?";
		int defaultPriceListId = new Query(commissionRun.getCtx(), MPriceList.Table_Name, whereClause, null)
			.setParameters( order.isSOTrx(), currencyId, commissionDefinition.get_ValueAsBoolean(AgencyChanges.SP027_IsTaxIncluded))
			.setOnlyActiveRecords(true)
			.setOrderBy("M_PriceList_ID")
			.setClient_ID()
			.firstId();
		if(defaultPriceListId <= 0) {
			throw new IllegalArgumentException("@M_PriceList_ID@ @NotFound@: @IsSOPriceList@: @" + order.isSOTrx() + "@, " +
				"@C_Currency_ID@: " + currency.getISO_Code()
				+ " @IsTaxIncluded@: @" + commissionDefinition.get_ValueAsBoolean(AgencyChanges.SP027_IsTaxIncluded) + "@");
		}
		order.setM_PriceList_ID(defaultPriceListId);
		...
		commissionRun.getCommissionAmtList().stream()
				.filter(commissionAmt -> commissionAmt.getCommissionAmt() != null
					...
					BigDecimal finalAmount = MConversionRate.convert(order.getCtx(), commissionAmt.getCommissionAmt().abs(), commissionRun.get_ValueAsInt("C_Currency_ID"), finalCurrencyId, order.getDateOrdered(), order.getC_ConversionType_ID(), order.getAD_Client_ID(), order.getAD_Org_ID());

					if (finalAmount == null) {
						throw new AdempiereException(
							MConversionRate.getErrorMessage(
								order.getCtx(),
								"NoCurrencyConversion",
								commissionRun.getC_Currency_ID(),
								finalCurrencyId,
					...
					orderLine.setPrice(finalAmount);
					orderLine.setTax();
	 ...
	 * @param amount
	 */
	private static void reversePreviousCommissionOrders(MInvoice sourceInvoice, BigDecimal amount, int reverseDocTypeId) {
		...
		new Query(sourceInvoice.getCtx(), I_C_Order.Table_Name, whereClause, sourceInvoice.get_TrxName())
			...
			.forEach(commissionOrderId -> {
				...
				reverseOrder.setC_DocTypeTarget_ID(reverseDocTypeId);
				reverseOrder.setC_DocType_ID(reverseDocTypeId);
