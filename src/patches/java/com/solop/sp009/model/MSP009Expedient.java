...
public class MSP009Expedient extends X_SP009_Expedient implements DocAction {
    ...
    /**
     * Create a SP009_ProductExpedient record for each distinct product invoiced under this Expedient
     */
    private void createProductExpedients() {
    	...
    	maybeInvoices.ifPresent(invoices -> invoices.forEach(invoice -> {
    		Arrays.asList(invoice.getLines()).forEach(invoiceLine -> {
    			boolean belongsToExpedient = invoiceLine.get_ValueAsInt(MSP009Expedient.COLUMNNAME_SP009_Expedient_ID) == getSP009_Expedient_ID()
    			...
    			if (belongsToExpedient && invoiceLine.getM_Product_ID() > 0)
    				productIds.add(invoiceLine.getM_Product_ID());
    		});
    	}));

    	productIds.forEach(this::createProductExpedient);
    }

    /**
     * Create the SP009_ProductExpedient record for the given product if it doesn't already exist
     * @param productId
     */
    private void createProductExpedient(int productId) {
    	String whereClause = X_SP009_ProductExpedient.COLUMNNAME_SP009_Expedient_ID.concat("=? AND ")
							.concat(X_SP009_ProductExpedient.COLUMNNAME_M_Product_ID).concat("=?");
    	X_SP009_ProductExpedient productExpedient = new Query(getCtx(), X_SP009_ProductExpedient.Table_Name, whereClause, get_TrxName())
    			.setParameters(getSP009_Expedient_ID(), productId)
    			.<X_SP009_ProductExpedient>first();
    	if (productExpedient != null)
    		return;
    	productExpedient = new X_SP009_ProductExpedient(getCtx(), 0, get_TrxName());
    	productExpedient.setAD_Org_ID(getAD_Org_ID());
    	productExpedient.setSP009_Expedient_ID(getSP009_Expedient_ID());
    	productExpedient.setM_Product_ID(productId);
    	productExpedient.saveEx();
    }
