...
public class ElectronicInvoicingUtil {
	public static File getPdfFileFromUrl(String downloadUrl) {
		try {
			...
			Pattern regex = Pattern.compile("[a-zA-Z\\d/_]*.pdf", Pattern.MULTILINE);
			Matcher regexMatcher = regex.matcher(firstHtmlPage);
			if (regexMatcher.find()) {
				validUrl = invoicyEndpoint + regexMatcher.group();
			}
			File validFile;
	...
	public static int getDocumentTypeFromTaxGroup(Properties context, String fiscalDocumentType, int taxGroupId, boolean isManual) {
		...
		if(taxGroup != null) {
			fiscalDocumentType = Optional.ofNullable(fiscalDocumentType).orElse(ElectronicInvoicingChanges.SP013_FiscalDocumentType_Invoice);
			if (isManual) {
