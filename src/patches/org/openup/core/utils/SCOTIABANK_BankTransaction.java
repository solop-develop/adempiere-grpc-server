 ...
 */
public class SCOTIABANK_BankTransaction extends BankTransactionAbstract {
    ...
    public void parseLine(String line) throws Exception {
        ...
        if (columns.length > 6 && !Util.isEmpty(columns[6])) {
            addValue(LINE_TRANSACTION_Description, columns[6].trim());
        }

        // Amount: col[10]
        if (columns.length > 10 && !Util.isEmpty(columns[10])) {
            String amountString = columns[10].replaceAll("\\+", "").trim();
            BigDecimal amount = getNumber(',', "#,###,###,###,###,###.##", amountString);
