...
public class MMovement extends X_M_Movement implements DocAction , DocumentReversalEnabled
{
	...
	public String completeIt()
	{
		...
		for (MMovementLine movementLine : movementLines)
		{
			...
			{
				MDDOrderLine oline = new MDDOrderLine(movementLine.getCtx(),movementLine.getDD_OrderLine_ID(), get_TrxName());
				MLocator locator_to = MLocator.get(movementLine.getCtx(), movementLine.getM_LocatorTo_ID());
				MWarehouse warehouse =  MWarehouse.get(movementLine.getCtx(), locator_to.getM_Warehouse_ID());
				if(warehouse.isInTransit()) {
					oline.setQtyInTransit(oline.getQtyInTransit().add(movementLine.getMovementQty()));
					oline.setConfirmedQty(Env.ZERO);
				} else {
					oline.setQtyInTransit(oline.getQtyInTransit().subtract(movementLine.getMovementQty()));
					oline.setQtyDelivered(oline.getQtyDelivered().add(movementLine.getMovementQty()));
				}
				oline.saveEx();
			}
		}	//	for all lines
		if(getDD_Order_ID() > 0) {
			...
			order.setIsInTransit(isInTranslating(order));
	...
	private boolean isInTranslating(MDDOrder order) {
