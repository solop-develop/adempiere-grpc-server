 ...
 */
public class BusinessPartnerServiceImplementation extends BusinessPartnerImplBase {
	/**	Logger			*/
	private CLogger log = CLogger.getCLogger(BusinessPartnerServiceImplementation.class);
	
	public String tableName = I_C_BPartner.Table_Name;

	@Override
	public void listBusinessPartnerInfo(ListBusinessPartnerInfoRequest request, StreamObserver<ListEntitiesResponse> responseObserver) {
		try {
			if(request == null) {
	 ...
	 * @return
	 */
	private ListEntitiesResponse.Builder convertEntitiesListFronGeneralInfo(Properties context, ListBusinessPartnerInfoRequest request) {		
