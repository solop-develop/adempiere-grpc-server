...
public class DictionaryUtil {
	 ...
	 * @return
	 */
	public static String getQueryWithReferencesFromColumns(MTable table, ArrayList<MColumn> columns) {
		String originalQuery = "SELECT FROM " + table.getTableName() + " AS " + table.getTableName() + " ";
		int fromIndex = originalQuery.toUpperCase().indexOf(" FROM ");
		StringBuffer queryToAdd = new StringBuffer(originalQuery.substring(0, fromIndex));
		StringBuffer joinsToAdd = new StringBuffer(originalQuery.substring(fromIndex, originalQuery.length() - 1));
		Language language = Language.getLanguage(Env.getAD_Language(Env.getCtx()));
		int index = 0;
		List<MColumn> columnsList = columns;
		if (columnsList == null || columnsList.size() < 1) {
			columnsList = table.getColumnsAsList();
		}
